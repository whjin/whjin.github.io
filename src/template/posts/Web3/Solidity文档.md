...
1. 线性化与函数覆盖 `override`
2. 多重继承中，某个合约的所有父合约集合中，合约与合约之间的继承关系构成了一个图结构，需要一种图的遍历算法来将这个图输出位一个线性结构。
3. 存储布局的堆叠、构造函数的执行顺序、`super`关键字的定义都依赖于这个结构。

### 继承

1. 继承定义 `is`关键字
2. 变量与函数、`modifier`、`event`的可见性
3. 构造函数
4. 继承中父子关系式确定的
5. 被继承的多个直接父合约之间用 `more-abstact-like` `to` `more-drived`也是确定的
6. 父在子前，兄在弟前，得出的排列顺序是确定的 `deterministic`

#### 继承的实现方式

1. 继承的实现方案是代码拷贝，合约继承后，部署到线上时，将变成一个合约。代码将从父合约拷贝到子合约中

### 多态

1. `virtual`与 `override`关键字
2. 可见性与 `mutability`的约束
3. `abstract`关键字 修饰合约

### `c3`线性序列化算法

1. 从当前合约开始找父合约，并且是从右向左遍历
2. **如果找到的合约的子合约都被遍历过**，该合约入列，否则回退一步，找兄合约（左侧合约）
3. 合约结束遍历，输出遍历顺序，并且倒置
