...
## `HTTP/2 vs HTTP/3` 核心特性差异

<!--prettier-ignore-start-->
|**特性**|`HTTP/2`|`HTTP/3`|
|---|---|---|
|底层协议|`TCP`|`QUIC`（基于 `UDP`）|
|队头阻塞|存在（`TCP`层全局阻塞）|不存在（流独立，单流丢包不影响其他）|
|连接建立|`TCP`三次握手 + `TLS`握手（`2RTT`）|`QUIC`合并握手（`0-RTT/1-RTT`）|
|连接迁移|不支持（`IP`/端口变化即断开）|支持（连接 `ID`标识），网络切换不中断|
|头部压缩|`HPACK`|`QPACK`（流独立压缩，更高效）|
|拥塞控制|固定 `TCP`算法|可插拔，针对弱网优化|
<!--prettier-ignore-end-->

## `HTTP/3`大模型场景

1. **大模型多模态对话**
   - 同时传输文本、音频、图片时无队头阻塞，避免大文件阻塞小请求
   - `0-RTT`握手显著降低首次响应延迟，提升对话流畅度
   - 移动网络切换时连接不中断，保证对话连续性
2. **医疗 `AI` 影像分析**
   - 大文件传输速度提升 `30%-50%`，减少影像上传等待时间
   - 弱网下丢包恢复能力更强，传输成功率更高
   - 支持断点续传，网络中断后无需重新上传整个文件

## 弱网环境 `AI` 请求不稳定解决方案

1. **协议层**
   - 配置 `HTTP/3`优先，自动降级到 `HTTP/2/1.1`
   - 开启 `BBR` 拥塞控制算法，提升弱网吞吐量
   - 启用 `0-RTT`握手，减少连接建立时间
2. **请求层**
   - 大请求分片传输，单分片失败仅重传该分片
   - 指数退避重试（`1s -> 2s -> 4s -> 8s`），避免服务器雪崩
   - 限流处理：解析响应头 `retry-After`字段，按服务器要求等待
...
<!--prettier-ignore-start-->
|<div style="text-align:center">内存缓存（`Map`）-></div>|<div style="text-align:center">`LocalStorage` -></div>|<div style="text-align:center">`IndexDB` -></div>|<div style="text-align:center">服务器缓存</div>|
|:---:|:---:|:---:|:---:|
|最快|较快|大容量|最可靠|
|`5`分钟过期|`7`天过期|`30`天过期|永久/按需|
<!--prettier-ignore-end-->

1. **一级缓存：内存缓存**
   - 存储：当前页面 `AI`答题结果、正在作答的记录、当前题目向量
   - 实现：使用 `Map`或状态管理库 `Zustand`
   - 过期：页面卸载或 `5`分钟后自动清楚

2. **二级缓存：`LocalStorage`**
   - 存储：最近 `7`天的 `AI`答题结果、常用题库向量
   - 注意：仅存储小数据 `<100KB`，避免阻塞主线程
   - 过期：写入时记录时间戳，读取时检查

3. **三级缓存：`IndexDB`**
   - 存储：所有题库向量、`30`天的答题记录、大体积 `AI`结果
   - 实现：使用 `localforage`简化操作
   - 过期：定期清理过期数据
