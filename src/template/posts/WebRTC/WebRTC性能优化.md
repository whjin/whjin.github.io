...
async function setBitRate() {
  ...
  } else if (lossRate < 3 && roundTripTime < 150 && availableBitrate > lastBritrate) {
    // 网络良好：升码率 10%
...
async function createAndSetOffer() {
  ...
  let modifiedSdp = offer.sdp.replace(/a=rtmpap:(\d+) H264\/\d+/g, (match, pt) => {
  ...
  // 启用 RTX （重传流）
  modifiedSdp = modifiedSdp.replace(/a=fmtp:(\d+) rtx\/\d+/g, (match, rtxPt) => {
    return `${match}\r\na=fmtp:${rtxPt} apt=96`; // 假设 H264 的 PT 为 96，需根据实际情况调整
  });

  await pc.setLocalDescription(new RTCSessionDescription({ type: 'offer', sdp: modifiedSdp }));
}
